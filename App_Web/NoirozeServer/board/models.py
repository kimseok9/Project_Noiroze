...
class CommunityBoard_Web(models.Model):                                                                       # 커뮤니티 게시판 모델
    CATEGORY_CHOICES = (
        ('정보공유', '정보공유'),
        ('소통해요', '소통해요'),
        ('칭찬해요', '칭찬해요'),
    ...
    like=models.ManyToManyField(settings.AUTH_USER_MODEL, related_name='like_web', blank=True)                                   # 추천 수
...
class Reply_web(models.Model):                                                                                      # 커뮤니티 게시판 댓글 모델
    community_board = models.ForeignKey(CommunityBoard_Web, on_delete=models.CASCADE)                               # 게시판 모델 외래키 설정
...
class ComplainBoard_Web(models.Model):                            # 민원접수 게시판 모델
    ...
    modify_date = models.DateTimeField('수정 일시', null=True, blank=True) 

    def __str__(self):
        return self.title


class Answer_web(models.Model):
    author = models.ForeignKey(CustomUser, on_delete=models.CASCADE)
    question = models.ForeignKey(ComplainBoard_Web, on_delete=models.CASCADE)
