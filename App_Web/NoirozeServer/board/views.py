...
def community_board_list(request):
    ...
    # 정렬 기준에 따라 게시물을 가져옴
    if so == 'recent':
        board_list = CommunityBoard_Web.objects.order_by('-created_date')
    elif so == 'late':
        board_list = CommunityBoard_Web.objects.order_by('created_date')
    elif so == 'recommend':
        board_list = CommunityBoard_Web.objects.annotate(num_voter=Count('voter')).order_by('-num_voter', '-created_date')
    elif so == 'popular':
        board_list = CommunityBoard_Web.objects.annotate(num_reply=Count('reply')).order_by('-num_reply', '-created_date')
    else:
        board_list = CommunityBoard_Web.objects.order_by('-id')
...
def create_community_board(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_board.save()
            return redirect('/board/community_board_list/')
    else:
...
def community_board_detail(request, pk):
    board = get_object_or_404(CommunityBoard_Web, pk=pk)
...
def create_complain_board(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_board.author = token.user
            new_board.save()
            return redirect('/board/complain_board_list/')
    else:
        form = ComplainBoardForm()
...
def update_complain_board(request, complain_board_id):
    complain_board = get_object_or_404(ComplainBoard_Web, id=complain_board_id)
...
def complain_board_list(request):
    ...
    if so == 'recent':
        board_list = ComplainBoard_Web.objects.order_by('-created_date')
    elif so == 'late':
        board_list = ComplainBoard_Web.objects.order_by('created_date')
    elif so == 'recommend':
        board_list = ComplainBoard_Web.objects.annotate(
        ...
        board_list = ComplainBoard_Web.objects.order_by('-id')
...
def complain_board_detail(request, pk):
    board = get_object_or_404(ComplainBoard_Web, pk=pk)
...
def answer_update(request, answer_id):
    answer = get_object_or_404(Answer_web, id=answer_id)
        ...
        if form.is_valid():
            ...
            try:
                ...
                if token.user == answer.author:
                    ...
                    updated_answer.save()
                    return redirect('board:complain_board_detail', pk = answer.question.id)
                else:
                    messages.error(request, '수정 권한이 없습니다.')
                    return redirect('board:complain_board_detail', pk = answer.question.id)
            except Token.DoesNotExist:
...
def answer_create(request, question_id):
    question = get_object_or_404(ComplainBoard_Web, pk=question_id)
    form = AnswerForm()
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            answer.save()
            return redirect('board:complain_board_detail', pk=question.id)
    else:
        form = AnswerForm()
...
def answer_delete(request, question_id, answer_id):
    if request.method == "POST":
        answer = get_object_or_404(Answer_web, pk=answer_id)
        token_key = request.COOKIES.get('auth-token')
        try:
            token = Token.objects.get(key=token_key)
        except Token.DoesNotExist:
    ...
    return redirect('board:complain_board_detail', pk=question_id)
